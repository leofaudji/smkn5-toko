...
trait BukuBesarDataTrait {
    ...
    public function fetchBukuBesarData(mysqli $conn, int $user_id, int $account_id, string $start_date, string $end_date): array
    {
        ...
        // 3. Ambil semua transaksi yang relevan dalam rentang tanggal dari JURNAL
        $query = "SELECT je.tanggal, je.keterangan, je.id as ref, jd.debit, jd.kredit FROM jurnal_details jd JOIN jurnal_entries je ON jd.jurnal_entry_id = je.id WHERE je.user_id = ? AND jd.account_id = ? AND je.tanggal BETWEEN ? AND ? ORDER BY je.tanggal ASC, je.id ASC";
